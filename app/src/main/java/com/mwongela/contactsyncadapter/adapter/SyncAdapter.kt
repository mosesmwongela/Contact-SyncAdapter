package com.mwongela.contactsyncadapter.adapter
...
import com.mwongela.contactsyncadapter.MainActivity
import com.mwongela.contactsyncadapter.entity.Contact
import com.mwongela.contactsyncadapter.util.Constants
import com.mwongela.contactsyncadapter.util.ContactsManager
...
class SyncAdapter(context: Context, autoInitialize: Boolean) :
    ...
    private val TAG: String = "SyncAdapter"
    private var mContactsList: ArrayList<Contact> = ArrayList()

    // A dummy list of valid numbers. From the server or from local db
    private var dummyServerResponseList: ArrayList<String> = arrayListOf(
        "254722111222" // add some numbers in your contact list here
    )

    // A list of valid numbers. From the server
    private var serverNumberList: ArrayList<String> = ArrayList()

    private val kenyanPhoneNumberPattern =
        Pattern.compile("^(?:254|\\+254|0)?(7[0-9]{8})$")

    init {
        Log.e(TAG, "SyncAdapter Created")
    ...
    override fun onPerformSync(
        ...
        for (contact in mContactsList) {
            for (number in contact.numbers) {
                ...
                Log.e(TAG, "NAME: " + contact.name + " Number: " + number)

                //process kenyan numbers only
                if (isKenyanPhoneNUmber(number)) {

                    var formattedNum = getFormattedNumber(number)

                    if (isNumberAlreadyRegistered(number)) {
                        // If number is registered and invalid on server, delete it
                        if (!dummyServerResponseList.contains(formattedNum)) {
                            ContactsManager.deleteNumber(context, formattedNum)
                        }
                    } else {
                        // If number is not registered and valid on server, register it
                        // This are the people with your app
                        if (dummyServerResponseList.contains(formattedNum)) {
    ...
    fun isKenyanPhoneNUmber(phone: String?): Boolean {
        return try {
            var phone1 = phone?.replace(" ".toRegex(), "")
            val matcher: Matcher = kenyanPhoneNumberPattern.matcher(phone1)
            matcher.matches()
